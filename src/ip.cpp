...
//	fprintf(stdout, "open socket %d %d %d\n", domain_, type_, protocol_);

	if (domain_ == LWIP_AF_INET)
    	s = lwip_socket (domain_, type_, protocol_);
	else
...
void zmq::unblock_socket (fd_t s_, bool is_lwip)
{
#if defined ZMQ_HAVE_WINDOWS
    u_long nonblock = 1;
...
#else
	if (is_lwip) {
    	int flags = lwip_fcntl (s_, F_GETFL, 0);
    	if (flags == -1)
        	flags = 0;
    	int rc = lwip_fcntl (s_, F_SETFL, flags | O_NONBLOCK);
    	errno_assert (rc != -1);
	}
	else {
...
void zmq::enable_ipv4_mapping (fd_t s_)
{
 ...
 //   int flag = 0;
#endif
//    int rc = lwip_setsockopt (s_, IPPROTO_IPV6, IPV6_V6ONLY, (const char*) &flag,
//        sizeof (flag));
#ifdef ZMQ_HAVE_WINDOWS
    wsa_assert (rc != SOCKET_ERROR);
#else
...
{
    int rc;
    struct lwip_sockaddr_storage ss;
...
#endif
    rc = lwip_getpeername (sockfd_, (struct lwip_sockaddr*) &ss, &addrlen);
#ifdef ZMQ_HAVE_WINDOWS
    if (rc == SOCKET_ERROR) {
...
//        return 0;
	...
	if (ss.ss_family == LWIP_AF_INET) {
		struct lwip_sockaddr_in *tmp = (struct lwip_sockaddr_in *)&ss;

		ip_addr_ = ipaddr_ntoa((ip_addr_t *)&(tmp->sin_addr));
	}
//	else {
//		struct lwip_sockaddr_in6 *tmp = (struct lwip_sockaddr_in6 *)&ss;
//
//		ip_addr_ = ip6addr_ntoa((ip6_addr_t *)&(tmp->sin6_addr));
//	}

    union {
        struct lwip_sockaddr sa;
        struct lwip_sockaddr_storage sa_stor;
    } u;
...
{
    int rc = lwip_setsockopt(s_, LWIP_IPPROTO_IP, LWIP_IP_TOS, reinterpret_cast<const char*>(&iptos), sizeof(iptos));
...
void zmq::bind_to_device (fd_t s_, std::string &bound_device_)
{
    ...
    int rc = lwip_setsockopt(s_, LWIP_SOL_SOCKET, LW_SO_BINDTODEVICE, bound_device_.c_str (), bound_device_.length ());
