...
int zmq::udp_engine_t::init (address_t *address_, bool send_, bool recv_)
{
    ...
    fd = open_socket (address->resolved.udp_addr->family (),
					LWIP_SOCK_DGRAM, LWIP_IPPROTO_UDP);
    if (fd == retired_fd)
        return -1;

    unblock_socket (fd, true);
...
void zmq::udp_engine_t::plug (io_thread_t* io_thread_, session_base_t *session_)
{
    ...
    if (send_enabled) {
        if (!options.raw_socket) {
            ...
            out_address = (lwip_sockaddr *) &raw_address;
            out_addrlen = sizeof (lwip_sockaddr_in);
        }
    ...
    if (recv_enabled) {
        int on = 1;
        int rc = lwip_setsockopt (fd, LWIP_SOL_SOCKET, LW_SO_REUSEADDR,
						(char *) &on, sizeof (on));
#ifdef ZMQ_HAVE_WINDOWS
        wsa_assert (rc != SOCKET_ERROR);
#else
        ...
        rc = lwip_bind (fd, address->resolved.udp_addr->bind_addr (),
...
void zmq::udp_engine_t::sockaddr_to_msg (zmq::msg_t *msg, lwip_sockaddr_in* addr)
{
    char* name = ipaddr_ntoa((ip_addr_t *)&(addr->sin_addr.s_addr));

    char port[6];
    sprintf (port, "%d", (int) lwip_ntohs (addr->sin_port));
...
int zmq::udp_engine_t::resolve_raw_address (char *name_, size_t length_)
{
    ...
    raw_address.sin_family = LWIP_AF_INET;
    raw_address.sin_port = lwip_htons (port);
    raw_address.sin_addr.s_addr = ipaddr_addr (addr_str.c_str ());

    if (raw_address.sin_addr.s_addr == IPADDR_NONE) {
        errno = EINVAL;
        return -1;
    }
...
void zmq::udp_engine_t::out_event()
{
    ...
    if (rc == 0) {
        ...
        wsa_assert (rc != SOCKET_ERROR);
#else
        rc = lwip_sendto (fd, out_buffer, size, 0, out_address, out_addrlen);
        errno_assert (rc != -1);
#endif
    }
    else
...
{
  struct lwip_sockaddr_in in_address;
  socklen_t in_addrlen = sizeof(lwip_sockaddr_in);
#ifdef ZMQ_HAVE_WINDOWS
...
#else
    int nbytes = lwip_recvfrom(fd, in_buffer, MAX_UDP_MSG, 0, (lwip_sockaddr*) &in_address, &in_addrlen);
    if (nbytes == -1) {
