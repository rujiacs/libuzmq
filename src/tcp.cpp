...
int zmq::tune_tcp_socket (fd_t s_)
{
    ...
    //  hurt latency.
    int nodelay = 1;
    int rc = lwip_setsockopt (s_, LWIP_IPPROTO_TCP, LWIP_TCP_NODELAY, (char*) &nodelay,
        sizeof (int));
    ...
    int nodelack = 1;
    rc = lwip_setsockopt (s_, LWIP_IPPROTO_TCP, LWIP_TCP_NODELACK, (char*) &nodelack,
        sizeof (int));
    tcp_assert_tuning_error (s_, rc);
#endif
    return rc;
...
int zmq::set_tcp_send_buffer (fd_t sockfd_, int bufsize_)
{
	...
	LIBZMQ_UNUSED(bufsize_);
	fprintf(stderr, "Warning: SO_SNDBUF is not supported by LwIP\n");
	return 0;	
//    const int rc = lwip_setsockopt (sockfd_, LWIP_SOL_SOCKET, SO_SNDBUF,
...
{
    const int rc = lwip_setsockopt (sockfd_, LWIP_SOL_SOCKET, LW_SO_RCVBUF,
        (char *) &bufsize_, sizeof bufsize_);
    tcp_assert_tuning_error (sockfd_, rc);
    return rc;
}
...
int zmq::tune_tcp_maxrt (fd_t sockfd_, int timeout_)
{
    ...
    tcp_assert_tuning_error (sockfd_, rc);
    return rc;
#elif defined (ZMQ_HAVE_TCP_USER_TIMEOUT)
				 ...
				 int is_data, bool is_hot)
{
	...
	if (is_data) {
		if (is_hot) {
    		nbytes = lwip_send_netml (s_, data_, size_,
						LWIP_MSG_NETML | LWIP_MSG_NETML_HOT, is_data);
		}
		else {
			nbytes = lwip_send_netml (s_, data_, size_,
						LWIP_MSG_NETML, is_data);
		}
	}
	else
		nbytes = lwip_send_netml (s_, data_, size_, 0, 0);
...
int zmq::tcp_read (fd_t s_, void *data_, size_t size_)
{
    ...
    const ssize_t rc = lwip_recv (s_, data_, size_, 0);
...
void zmq::tcp_assert_tuning_error (zmq::fd_t s_, int rc_)
{
    ...
    int rc = lwip_getsockopt (s_, LWIP_SOL_SOCKET, LW_SO_ERROR, (char*) &err, &len);
