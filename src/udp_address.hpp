...
#endif
        const lwip_sockaddr *bind_addr () const;
        socklen_t bind_addrlen () const;

        const lwip_sockaddr *dest_addr () const;
        socklen_t dest_addrlen () const;

        bool is_mcast () const;

        const lwip_in_addr multicast_ip () const;
        const lwip_in_addr interface_ip () const;

    private:
        lwip_in_addr  multicast;
        lwip_in_addr  iface;
        lwip_sockaddr_in bind_address;
        lwip_sockaddr_in dest_address;
        bool is_multicast;
        std::string address;
    };
}
