...
int zmq::tcp_connecter_t::open ()
{
    ...
    //  Create the socket.
    s = open_socket (tcp_addr->family (), LWIP_SOCK_STREAM, LWIP_IPPROTO_TCP);

    //  IPv6 address family not supported, try automatic downgrade to IPv4.
    if (s == zmq::retired_fd && tcp_addr->family () == LWIP_AF_INET6
    && errno == EAFNOSUPPORT
    && options.ipv6) {
    ...
    if (tcp_addr->family () == LWIP_AF_INET6)
    ...
    if (tcp_addr->has_src_addr ()) {
        ...
        rc = lwip_bind (s, tcp_addr->src_addr (), tcp_addr->src_addrlen ());
        if (rc == -1)
            return -1;
    }
	...
	if (socket->get_localid() == SCHEDULAR_ID ||
		...
		lwip_setbypass(s);
		fprintf(stdout, "[%s][%d]: sock %d (%u->%u) is set to bypass\n",
						__FILE__, __LINE__, s,
						socket->get_localid(), socket->get_remoteid());
	}
    //  Connect to the remote peer.
    rc = lwip_connect (s, tcp_addr->addr (), tcp_addr->addrlen ());
...
zmq::fd_t zmq::tcp_connecter_t::connect ()
{
    ...
    const int rc = lwip_getsockopt (s, LWIP_SOL_SOCKET, LW_SO_ERROR, (char*) &err, &len);
