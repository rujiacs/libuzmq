...
//	fprintf(stdout, "[%s][%d]: accept sock %d, %u->%u\n",
//					__FILE__, __LINE__, fd,
//					socket->get_localid(), socket->get_remoteid());
...
int zmq::tcp_listener_t::get_address (std::string &addr_)
{
...
#endif
    int rc = lwip_getsockname (s, (struct lwip_sockaddr *) &ss, &sl);
    ...
    tcp_address_t addr ((struct lwip_sockaddr *) &ss, sl);
...
int zmq::tcp_listener_t::set_address (const char *addr_)
{
	...
	int type = LWIP_SOCK_STREAM, domain = address.family(), proto = LWIP_IPPROTO_TCP;
    //  Create a listening socket.
    s = open_socket (domain, type, proto);

    //  IPv6 address family not supported, try automatic downgrade to IPv4.
    if (s == zmq::retired_fd && address.family () == LWIP_AF_INET6
    && errno == EAFNOSUPPORT
    && options.ipv6) {
        rc = address.resolve (addr_, true, false);
        if (rc != 0)
            return rc;
        s = open_socket (LWIP_AF_INET, LWIP_SOCK_STREAM, LWIP_IPPROTO_TCP);
    }
    ...
    if (address.family () == LWIP_AF_INET6)
    ...
    wsa_assert (rc != SOCKET_ERROR);
#else
    rc = lwip_setsockopt (s, LWIP_SOL_SOCKET, LW_SO_REUSEADDR, &flag, sizeof (int));
    errno_assert (rc == 0);
#endif

    //  Bind the socket to the network interface and port.
    rc = lwip_bind (s, address.addr (), address.addrlen ());
#ifdef ZMQ_HAVE_WINDOWS
    if (rc == SOCKET_ERROR) {
        errno = wsa_error_to_errno (WSAGetLastError ());
        goto error;
    ...
    rc = lwip_listen (s, options.backlog);
#ifdef ZMQ_HAVE_WINDOWS
    if (rc == SOCKET_ERROR) {
        errno = wsa_error_to_errno (WSAGetLastError ());
        goto error;
...
#else
    fd_t sock = lwip_accept (s, (struct lwip_sockaddr *) &ss, &ss_len);
#endif
...
//#endif
    ...
    if (!options.tcp_accept_filters.empty ()) {
        ...
        for (options_t::tcp_accept_filters_t::size_type i = 0; i != options.tcp_accept_filters.size (); ++i) {
            if (options.tcp_accept_filters[i].match_address ((struct lwip_sockaddr *) &ss, ss_len)) {
                matched = true;
                break;
            }
        }
