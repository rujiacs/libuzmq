...
#define PROXY_CLEANUP()\
    do {\
        if (poller_all) { delete poller_all; poller_all = NULL; } \
        if (poller_in) { delete poller_in; poller_in = NULL; } \
        if (poller_control) { delete poller_control; poller_control = NULL; } \
        if (poller_receive_blocked) { delete poller_receive_blocked; poller_receive_blocked = NULL; } \
        if (poller_send_blocked) { delete poller_send_blocked; poller_send_blocked = NULL; } \
        if (poller_both_blocked) { delete poller_both_blocked; poller_both_blocked = NULL; } \
        if (poller_frontend_only) { delete poller_frontend_only; poller_frontend_only = NULL; } \
        if (poller_backend_only) { delete poller_backend_only; poller_backend_only = NULL; } \
    } while (false)
