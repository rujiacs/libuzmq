...
int zmq_global_init(const char *ip, const char *gw, const char *mask)
{
	return zmq_lwip_init(ip, gw, mask);
}
...
int zmq_set_localid(void *s_, uint16_t id) {
...
int zmq_set_remoteid(void *s_, uint16_t id) {
	zmq::socket_base_t *s = static_cast<zmq::socket_base_t *> (s_);
...
int zmq_msg_init_data (zmq_msg_t *msg_, void *data_, size_t size_,
    zmq_free_fn *ffn_, void *hint_, uint16_t id_)
{
    return ((zmq::msg_t*) msg_)->init_data (data_, size_, ffn_, hint_, id_);
}
