...
int zmq::tcp_address_t::resolve_nic_name(const char *nic, bool ipv6, bool is_src)
{
	LIBZMQ_UNUSED(nic);
	LIBZMQ_UNUSED(ipv6);
	LIBZMQ_UNUSED(is_src);
...
int zmq::tcp_address_t::resolve_nic_name(const char *nic_, bool ipv6_, bool is_src_)
{
    ...
    if (rc == 0) {
        ...
        while (current_addresses) {
            ...
            if (str_rc1 == 0) xfree(if_name);
            if (str_rc2 == 0) xfree(if_friendly_name);
...
int zmq::tcp_address_t::resolve_interface (const char *interface_, bool ipv6_, bool is_src_)
{
    ...
    lwip_sockaddr_storage ss;
    lwip_sockaddr *out_addr = (lwip_sockaddr*) &ss;
    size_t out_addrlen;

    //  Initialise IP-format family/port and populate temporary output pointers
    //  with the address.
    if (ipv6_) {
//        lwip_sockaddr_in6 ip6_addr;
//        memset (&ip6_addr, 0, sizeof (struct lwip_in6_addr) );
//        ip6_addr.sin6_family = LWIP_AF_INET6;
//        memcpy (&ip6_addr.sin6_addr, &lwip_in6addr_any, sizeof (lwip_in6addr_any) );
//        out_addrlen = sizeof (ip6_addr);
//        memcpy (out_addr, &ip6_addr, out_addrlen);
    }
    else {
        lwip_sockaddr_in ip4_addr;
        memset (&ip4_addr, 0, sizeof (struct lwip_in_addr) );
        ip4_addr.sin_family = LWIP_AF_INET;
        ip4_addr.sin_addr.s_addr = lwip_htonl (IPADDR_ANY);
	...
	req.ai_family = LWIP_AF_INET;
//    req.ai_family = ipv6_? LWIP_AF_INET6: LWIP_AF_INET;

    //  Arbitrary, not used in the output, but avoids duplicate results.
    req.ai_socktype = LWIP_SOCK_STREAM;

    //  Restrict hostname/service to literals to avoid any DNS lookups or
    //  service-name irregularity due to indeterminate socktype.
    req.ai_flags = LWIP_AI_PASSIVE | LWIP_AI_NUMERICHOST;

    //  Resolve the literal address. Some of the error info is lost in case
    //  of error, however, there's no way to report EAI errors via errno.

    rc = lwip_getaddrinfo(interface_, NULL, &req, &res);
...
//	struct lwip_sockaddr_in *tmp = (struct lwip_sockaddr_in *)res->ai_addr;

//	fprintf(stderr, "after resolve_interface, addrlen %u, ip %u, family %u, port %u\n",
//					res->ai_addrlen, tmp->sin_addr.s_addr, tmp->sin_family, tmp->sin_port);
...
//    req.ai_family = ipv6_? LWIP_AF_INET6: LWIP_AF_INET;
    ...
    lwip_addrinfo *res;
#endif
    int rc = lwip_getaddrinfo (hostname_, NULL, &req, &res);
...
zmq::tcp_address_t::tcp_address_t (const lwip_sockaddr *sa, socklen_t sa_len) :
    _has_src_addr (false)
{
    ...
    memset (&address, 0, sizeof (address) );
    memset (&source_address, 0, sizeof (source_address) );
    if (sa->sa_family == LWIP_AF_INET && sa_len >= (socklen_t) sizeof (address.ipv4) )
        memcpy (&address.ipv4, sa, sizeof (address.ipv4) );
//    else
//    if (sa->sa_family == LWIP_AF_INET6 && sa_len >= (socklen_t) sizeof (address.ipv6) )
...
#if !defined ZMQ_HAVE_WINDOWS_TARGET_XP && !defined ZMQ_HAVE_WINDOWS_UWP 
            zone_id = lwip_if_nametoindex(if_str.c_str());
#else
...
//	fprintf(stdout, "[%s][%d]: addr %s, port %u\n", __FILE__, __LINE__,
//					addr_str.c_str(), port);
    ...
    if (is_src_) {
//        if (source_address.generic.sa_family == LWIP_AF_INET6) {
//            source_address.ipv6.sin6_port = lwip_htons (port);
//            source_address.ipv6.sin6_scope_id = zone_id;
//        }
//        else
            source_address.ipv4.sin_port = lwip_htons (port);
//			fprintf(stdout, "end of resolve, addr %u, port %u, family %u\n",
//					source_address.ipv4.sin_addr.s_addr,
//					source_address.ipv4.sin_port,
//					source_address.ipv4.sin_family);
    }
    else {
//        if (address.generic.sa_family == LWIP_AF_INET6) {
//            address.ipv6.sin6_port = lwip_htons (port);
//            address.ipv6.sin6_scope_id = zone_id;
//        }
//        else
            address.ipv4.sin_port = lwip_htons (port);
//			fprintf(stdout, "end of resolve, addr %u, port %u, family %u\n",
//					address.ipv4.sin_addr.s_addr,
//					address.ipv4.sin_port,
//					address.ipv4.sin_family);
    }
...
int zmq::tcp_address_t::to_string (std::string &addr_)
{
    ...
    if (address.generic.sa_family != LWIP_AF_INET) {
//    &&  address.generic.sa_family != LWIP_AF_INET6) {
        addr_.clear ();
        return -1;
    }
...
//    if (address.generic.sa_family == LWIP_AF_INET6) {
//        std::stringstream s;
//        s << "tcp://[" << ip6addr_ntoa((ip6_addr_t *)address.ipv6.sin6_addr.lwip_s6_addr)
//				<< "]:" << lwip_ntohs (address.ipv6.sin6_port);
//        addr_ = s.str ();
//    }
//    else {
        std::stringstream s;
        s << "tcp://" << ipaddr_ntoa((ip_addr_t *)&(address.ipv4.sin_addr.s_addr))
				<< ":" << lwip_ntohs (address.ipv4.sin_port);
        addr_ = s.str ();
//    }
    return 0;
}

const lwip_sockaddr *zmq::tcp_address_t::addr () const
{
...
const lwip_sockaddr *zmq::tcp_address_t::src_addr () const
{
...
#else
lwip_sa_family_t zmq::tcp_address_t::family () const
#endif
{
    return address.ipv4.sin_family;
}
...
//        else
    ...
    if (mask_str == "0")
        ...
        if (
            (mask < 1) ||
//            (address.generic.sa_family == LWIP_AF_INET6 && mask > 128) ||
            (address.generic.sa_family != LWIP_AF_INET6 && mask > 32)
        ) {
            errno = EINVAL;
            return -1;
        }
...
//    if (address.generic.sa_family == LWIP_AF_INET6) {
//        std::stringstream s;
//        s << "[" << ip6addr_ntoa((ip6_addr_t *)address.ipv6.sin6_addr.lwip_s6_addr)
//				<< "]/" << address_mask;
//        addr_ = s.str ();
//    }
//    else {
        std::stringstream s;
        s << ipaddr_ntoa((ip_addr_t *)&(address.ipv4.sin_addr.s_addr)) << "/" << address_mask;
        addr_ = s.str ();
//    }
    return 0;
}

bool zmq::tcp_address_mask_t::match_address (const struct lwip_sockaddr *ss,
				const socklen_t ss_len) const
{
    zmq_assert (address_mask != -1
             && ss != NULL
             && ss_len >= (socklen_t) sizeof (struct lwip_sockaddr));
...
//        if (ss->sa_family == LWIP_AF_INET6) {
//            zmq_assert (ss_len == sizeof (struct lwip_sockaddr_in6));
//            their_bytes = (const uint8_t *) &(((const struct lwip_sockaddr_in6 *) ss)->sin6_addr);
//            our_bytes = (const uint8_t *) &address.ipv6.sin6_addr;
//            mask = sizeof (struct lwip_in6_addr) * 8;
//        }
//        else {
            zmq_assert (ss_len == sizeof (struct lwip_sockaddr_in));
            their_bytes = (const uint8_t *) &(((const struct lwip_sockaddr_in *) ss)->sin_addr);
            our_bytes = (const uint8_t *) &address.ipv4.sin_addr;
            mask = sizeof (struct lwip_in_addr) * 8;
//        }
